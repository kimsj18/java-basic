...
//        System.out.println("나는 CircleCal1의 areaCilcle 메소드야");
//        super.areaCirCle(20);
//    return Math.PI*radius *radius;
//}

    //컴파일 단계에서 정확히 부모의 메소드를 오버라이딩이 되었는지 체크하고 문제가 있으면 컴파일에러를 출력한다.
    @Override       //@(at) 어노테이션 마크: 개발자의 실수를 줄여주기 위해 정확히 오버라이딩이 되었는지 체크
        public double areaCirCle(double radius) {
        double result = super.areaCirCle(radius);
        System.out.println("부모의 버전 계산 : "+result);  //자식메소드와 부모메소드의 공동 작업처리 기법
